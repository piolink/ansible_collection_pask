...
class PaskPrescript(PaskModule):
    def __init__(self, name, module_args, required_if):
        super(PaskPrescript, self).__init__(name, module_args, required_if)
...
def main():
    prescript = PaskPrescript(name, module_args, required_if)
    prescript.set_param()
    prescript.run()
    prescript.set_result()
