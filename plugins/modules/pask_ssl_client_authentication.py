...
client_authentication_export_inner_args = dict(
...
module_args = dict(
    ...
    trusted_certificate_text=dict(type='str'),
    import_crt=dict(type='str'),
    export_crt=dict(type='list', elements='dict',
                    options=client_authentication_export_inner_args),
...
name = 'ssl'
subname = 'client-authentication'

command_list = ['export_crt', 'export_crl', 'import_crl']

class PaskSslClientAuthentication(PaskModule):
    def __init__(self, name, module_args, required_if):
        super(PaskSslClientAuthentication,
              self).__init__(name, module_args, required_if)
...
def main():
    ssl_client_authentication = PaskSslClientAuthentication(
        name, module_args, required_if)
    ssl_client_authentication.set_param()
    ssl_client_authentication.run()
    ssl_client_authentication.set_result()
