...
param_list = [
    ...
    'passive_health_check', 'front_end_optimize', 'snat_addr_type',
    'fail_action'
]
module_args = dict(
    name=dict(type='str', required=True),
    ...
    request_buffering=dict(type='dict',
                           options=request_buffering_inner_args),
    ...
    rule=dict(type='list', elements='dict',
              options=rule_inner_args),
    cookie_encryption=dict(type='dict',
                           options=cookie_encryption_inner_args)
)
...
class PaskAdvl7slb(PaskModule):
    def __init__(self, name, module_args, required_if):
        super(PaskAdvl7slb, self).__init__(name, module_args, required_if)
...
def main():
    advl7slb = PaskAdvl7slb(name, module_args, required_if)
    advl7slb.set_param()
    advl7slb.run()
    advl7slb.set_result()
