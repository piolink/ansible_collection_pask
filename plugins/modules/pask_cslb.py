...
param_list = [
    ...
    'session_timeout_mode', 'session_reset', 'active_nodest', 'fail_skip',
    'priority', 'lb_method', 'reverse_only', 'recording_cache',
    'transparent_cache', 'reset_reuse_connection', 'graceful_reverse_service'
]
...
class PaskCslb(PaskModule):
    def __init__(self, name, module_args):
        super(PaskCslb, self).__init__(name, module_args)
...
def main():
    cslb = PaskCslb(name, module_args)
    cslb.set_param()
    cslb.run()
    cslb.set_result()
