#!/usr/bin/python
# -*- coding: utf-8 -*-

# Copyright: (c) 2020, Piolink Inc.
# GNU General Public License v3.0+ (see COPYING or https://www.gnu.org/licenses/gpl-3.0.txt)

from __future__ import absolute_import, division, print_function
__metaclass__ = type
from ansible_collections.piolink.pask.plugins.module_utils.pask_module import PaskModule,\
    make_module_args, try_except
import os


param_list = [
    'proxy_ndp'
]
static_inner_args = dict(
    ip=dict(type='str', required=True),
    mac=dict(type='str', required=True)
)
proxy_neighbor_inner_args = dict(
    ip=dict(type='str', required=True),
    interface=dict(type='list', elements='str', required=True)
)
module_args = dict(
    static=dict(type='list', elements='dict', options=static_inner_args),
    proxy_neighbor=dict(type='list', elements='dict',
                        options=proxy_neighbor_inner_args),
)


module_args.update(make_module_args(param_list))

name = 'neighbor'


class PaskNeighbor(PaskModule):
    def __init__(self, name, module_args):
        super(PaskNeighbor, self).__init__(name, module_args)

    @try_except
    def run(self):
        data = self.make_data(self.module.params, include_inner=True)
        self.resp = self.prest.put(self.url, data)


def main():
    neighbor = PaskNeighbor(name, module_args)
    neighbor.set_param()
    neighbor.run()
    neighbor.set_result()


if __name__ == '__main__':
    main()